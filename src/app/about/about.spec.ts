import { ComponentFixture, TestBed } from '@angular/core/testing';
import { Observable, of, Subject, throwError } from 'rxjs';
import { About, SECTION_TITLE, CV_BUTTON_LABEL } from './about';
import { PortfolioService } from '../shared/services/portfolio.service';
import { Portfolio } from '../shared/models/portfolio.models';

describe('About', () => {
  let portfolioService: { getPortfolio: () => Observable<Portfolio> };

  beforeEach(() => {
    portfolioService = { getPortfolio: vi.fn() };
  });

  it('renders section with id sobre-mi', () => {
    const { compiled } = setup(() => of(mockPortfolio()));
    expect(compiled.querySelector('section#sobre-mi')).toBeTruthy();
  });

  it('shows skeleton while loading', () => {
    const { compiled } = setup(() => new Subject<Portfolio>().asObservable());
    expect(compiled.querySelectorAll('.animate-pulse').length).toBeGreaterThan(0);
  });

  it('renders title, divider, labels and values on success', () => {
    const portfolio = mockPortfolio();
    const { compiled } = setup(() => of(portfolio));

    const heading = compiled.querySelector('h2');
    expect(heading?.textContent?.trim()).toBe(SECTION_TITLE);
    expect(compiled.querySelector('.h-1.w-12.bg-primary')).toBeTruthy();

    expect(compiled.textContent).toContain('Correo');
    expect(compiled.textContent).toContain('Ubicación');
    expect(compiled.textContent).toContain('Teléfono');
    expect(compiled.textContent).toContain('Edad');

    expect(compiled.textContent).toContain(portfolio.about.email);
    expect(compiled.textContent).toContain(portfolio.about.ubicacion);
    expect(compiled.textContent).toContain(portfolio.about.telefono);
    expect(compiled.textContent).toContain(portfolio.about.edad);
  });

  it('renders email anchor with mailto href', () => {
    const portfolio = mockPortfolio();
    const { compiled } = setup(() => of(portfolio));

    const emailLink = compiled.querySelector<HTMLAnchorElement>('a[href^="mailto:"]');
    expect(emailLink?.getAttribute('href')).toBe(`mailto:${portfolio.about.email}`);
    expect(emailLink?.textContent?.trim()).toBe(portfolio.about.email);
  });

  it('renders phone anchor with tel href', () => {
    const portfolio = mockPortfolio();
    const { compiled } = setup(() => of(portfolio));

    const phoneLink = compiled.querySelector<HTMLAnchorElement>('a[href^="tel:"]');
    expect(phoneLink?.getAttribute('href')).toBe(`tel:${portfolio.about.telefono}`);
    expect(phoneLink?.textContent?.trim()).toBe(portfolio.about.telefono);
  });

  it('renders CV button with href and download attribute', () => {
    const portfolio = mockPortfolio();
    const { compiled } = setup(() => of(portfolio));

    const cvLink = compiled.querySelector<HTMLAnchorElement>('a[download]');
    expect(cvLink?.getAttribute('href')).toBe(portfolio.about.cv);
    expect(cvLink?.textContent?.trim()).toBe(CV_BUTTON_LABEL);
  });

  it('does not render about.descripcion', () => {
    const portfolio = mockPortfolio();
    const { compiled } = setup(() => of(portfolio));
    expect(compiled.textContent).not.toContain(portfolio.about.descripcion);
  });

  it('shows error message and retry button when fetch fails', () => {
    const { compiled } = setup(() => throwError(() => new Error('API error')));

    expect(compiled.textContent).toContain('No se pudo cargar la información');
    const retryButton = compiled.querySelector<HTMLButtonElement>('button');
    expect(retryButton?.textContent?.trim()).toBe('Reintentar');
  });

  it('reloads portfolio when retry button is clicked', async () => {
    const { fixture, compiled } = setup(() => throwError(() => new Error('API error')));
    const retryButton = compiled.querySelector<HTMLButtonElement>('button');

    retryButton?.click();
    await fixture.whenStable();

    expect(portfolioService.getPortfolio).toHaveBeenCalledTimes(2);
  });

  function setup(serviceFactory: () => Observable<Portfolio>): {
    fixture: ComponentFixture<About>;
    compiled: HTMLElement;
  } {
    portfolioService.getPortfolio = vi.fn(serviceFactory);

    TestBed.configureTestingModule({
      imports: [About],
      providers: [{ provide: PortfolioService, useValue: portfolioService }],
    });

    const fixture = TestBed.createComponent(About);
    fixture.detectChanges();
    return { fixture, compiled: fixture.nativeElement as HTMLElement };
  }
});

function mockPortfolio(): Portfolio {
  return {
    about: {
      nombreCompleto: 'Aldo Guevara Muñoz',
      descripcion: 'Desarrollador full stack junior especializado en Angular y .NET.',
      email: 'test@example.com',
      ubicacion: 'Ciudad de México',
      telefono: '5555555555',
      edad: '25',
      cv: '/assets/documents/CV_Aldo_Guevara_Muñoz.pdf',
      foto: '/assets/images/profile.png',
    },
    experiencia: [],
    proyectos: [],
    habilidades: [],
    educacion: [],
  };
}
